...
# import 
from UCHIHA.misc import SUDOERS as SUDO_USER
from UCHIHA.cplugin.utils.data import RAID, PBIRAID, OneWord, HIRAID, PORM, EMOJI, GROUP, VERIFIED_USERS
...
async def raid(Client: Client, m: Message):  
      ARJUN = "".join(m.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(ARJUN) == 2:
        counts = int(ARJUN[0])
        username = ARJUN[1]
        if not counts:
        ...
        if not username:
          ...
          return
        try:
           user = await Client.get_users(ARJUN[1])
        except:
